...
if (!gotTheLock) {
  ...
  app.on('ready', () => {
    ...
    // We pass in the Application User Model ID here. This is generated by Squirrel from name property in the maker config
    // and the name property in package.json. This should be embedded in the app shortcut, and is checkable with
    // the lnk-parser tool: https://code.google.com/archive/p/lnk-parser/ under the System.AppUserModel.ID property.
    // This allows Windows to attribute the notification to the correct app (and show the app icon with the notification).
    const notifier = ToastNotificationManager.createToastNotifier('com.squirrel.MyApp.electron-protocol-notification');
    notifier.show(toast);
  });
